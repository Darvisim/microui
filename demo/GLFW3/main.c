...
static void mouse_button_callback(GLFWwindow *window, int button,
  ...
  if (b && action == GLFW_PRESS) {
    mu_input_mousedown(g_ctx, (int)x_pos, (int)y_pos, b);
  }
  if (b && action == GLFW_RELEASE) {
    mu_input_mouseup(g_ctx, (int)x_pos, (int)y_pos, b);
  }
}

static void mouse_motion_callback(GLFWwindow *window, double x_pos, double y_pos) {
  mu_input_mousemove(g_ctx, (int)x_pos, (int)y_pos);
}
...
static void key_callback(GLFWwindow *window, int key, int scancode,
  ...
  if (c && action == GLFW_PRESS) {
    mu_input_keydown(g_ctx, c);
  }
  if (c && action == GLFW_RELEASE) {
    mu_input_keyup(g_ctx, c);
  }
}

int main(int argc, char **argv) {
  /* init GLFW and renderer */
  GLFWwindow *window = r_init("GLFW window");
  if (!window) {
    return -1;
  }
